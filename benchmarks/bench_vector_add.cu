...
struct VectorAddResult
{
    ...
    std::string version{}; // implementation version, e.g. `cpu_serial`, `cuda_naive`, `pinned memcpy`
...
VectorAddResult make_vector_add_result(
...
VectorAddResult bench_vector_add_cpu(
    ...
    CpuTimer& cpu_timer)
{
    VectorAddResult r = make_vector_add_result(
        config.kernel,
        "cpu_serial",
        "cpu",
        config.dtype,
...
VectorAddResult bench_vector_add_h2d(
    const BenchmarkConfig& config,
    const float* a_host,
    const float* b_host,
    float* a_dev,
...
VectorAddResult bench_vector_add_kernel(
    const BenchmarkConfig& config,
    const float* a_dev,
    const float* b_dev,
    float* result_dev,
    const float* result_host,
    float* result_from_dev,
    size_t n,
    int block_size,
...
VectorAddResult bench_vector_add_d2h(
    const BenchmarkConfig& config,
    const float* result_dev,
    float* result_from_dev,
    size_t n,
    CpuTimer& cpu_timer)
{
...
void run_kernel_block_size_sweep(
    std::ofstream& out,
    const BenchmarkConfig& config,
    const float* a_dev,
    const float* b_dev,
    ...
    CudaTimer& cuda_timer)
{
    ...
    {
        VectorAddResult result_kernel = bench_vector_add_kernel(
            config,
            a_dev,
            b_dev,
            result_dev,
    ...
    CpuTimer& cpu_timer)
{
    ...
    // CPU benchmark
    VectorAddResult result_cpu = bench_vector_add_cpu(
        config,
        a_host.get(),
        b_host.get(),
        result_host.get(),
    ...
    // H2D benchmark
    VectorAddResult result_h2d = bench_vector_add_h2d(
        config,
        a_host.get(),
        b_host.get(),
        a_dev.get(),
    ...
    // D2H benchmark
    VectorAddResult result_d2h = bench_vector_add_d2h(
        config,
        result_dev.get(),
        result_from_dev.get(),
        n,
